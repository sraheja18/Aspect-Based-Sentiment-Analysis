...
class bert_ATE(torch.nn.Module):
    ...
    def forward(self, ids_tensors, tags_tensors, masks_tensors):
        bert_outputs= self.bert(input_ids=ids_tensors, attention_mask=masks_tensors,return_dict=False)[0]
...
class bert_ABSA(torch.nn.Module):
    ...
    def forward(self, ids_tensors, label_tensors, masks_tensors, segments_tensors):
        bert_outputs = self.bert(input_ids=ids_tensors, attention_mask=masks_tensors, token_type_ids=segments_tensors, return_dict = False)[1]
        ...
        if label_tensors is not None:
            # print(linear_outputs.size())
            # print(tags_tensors.size())
            loss = self.loss_fn(linear_outputs, label_tensors)
            return loss
        else:
            return linear_outputs